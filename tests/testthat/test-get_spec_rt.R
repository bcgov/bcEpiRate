...
test_that("function throws an error when either vector contains negative elements", {
  ...
  expect_error(get_spec_rt(counts_one_neg, popn), "`counts` should contain non-negative elements")
  expect_error(get_spec_rt(counts_all_neg, popn), "`counts` should contain non-negative elements")
  expect_error(get_spec_rt(counts, popn_one_neg), "`popn` should contain non-negative elements")
  expect_error(get_spec_rt(counts, popn_all_neg), "`popn` should contain non-negative elements")
})

test_that("function throws an error when vectors aren't the same length", {
  expect_error(get_spec_rt(counts, popn[1:4]), "`counts` and `popn` must be the same length")
})

test_that("function throws an error when `scale` and `power` are supplied", {
  expect_error(get_spec_rt(counts, popn, scale = 100000, power = 5), "must supply exactly one of `scale` and `power`")
})

test_that("function throws an error when `scale` isn't a single number", {
  expect_error(get_spec_rt(counts, popn, scale = c(1, 1000)), "`scale` must be a single number")
})

test_that("function throws an error when `scale` isn't a positive whole number", {
  expect_error(get_spec_rt(counts, popn, scale = 0), "`scale` must be a positive whole number")
  expect_error(get_spec_rt(counts, popn, scale = -1), "`scale` must be a positive whole number")
  expect_error(get_spec_rt(counts, popn, scale = 0.1), "`scale` must be a positive whole number")
})

test_that("function throws an error when `power` isn't a single number", {
  expect_error(get_spec_rt(counts, popn, power = c(1, 2, 3)), "`power` must be a single number")
})

test_that("function throws an error when `power` isn't a non-negative whole number", {
  expect_error(get_spec_rt(counts, popn, power = -1), "`power` must be a non-negative whole number")
  expect_error(get_spec_rt(counts, popn, power = 0.1), "`power` must be a non-negative whole number")
})
...
test_that("function raises both warnings, if necessary", {
  ...
  expect_equal(warnings[1], "one or more elements in `counts` and/or `popn` are NA, pass `output_status = TRUE` to check")
  expect_equal(warnings[2], "one or more elements in `popn` are 0, pass `output_status = TRUE` to check")
})

test_that("function ouputs expected statuses", {
  counts_2 <- c(0, 0, 0, NA)
  popn_2 <- c(0, NA, 10, 0)
  counts_3 <- c(NA, 10, 100, 1000)
  popn_3 <- c(10, 0, NA, 10000)
  ...
  expect_equal(
    get_spec_rt(counts_2, popn_2, output_status = TRUE)$status,
    c("denom_0", "has_NA", "success", "has_NA")
  )
  expect_equal(
    get_spec_rt(counts_3, popn_3, output_status = TRUE)$status,
...
test_that("function throws a warning when the use the normal distribution should be reconsidered", {
  expect_warning(
...
test_that("limits are NA when specific rates evaluate to 0", {
  ...
  # supressing warning saying this distribution should be avoided
  suppressWarnings(get_spec_rt(counts_0, popn, power = 3, dist = "normal", interval = 0.95)) %>%
    dplyr::mutate(is_expected = dplyr::if_else(
      rate == 0,
      is.na(lower) & is.na(upper), # if rate is 0, limits are both NA
      is.finite(lower) & is.finite(upper)
    )) %>% # otherwise, limits are both finite
...
test_that("limits are NA when specific rates evaluate to NA", {
  ...
  suppressWarnings(get_spec_rt(c(1, NA, NA, 4, 5), popn, power = 3, dist = "normal", interval = 0.9)) %>%
    dplyr::mutate(is_expected = dplyr::if_else(
      is.na(rate),
      is.na(lower) & is.na(upper), # if rate is NA, so are the limits
      is.finite(lower) & is.finite(upper)
    )) %>% # otherwise, limits should be finite numbers
    dplyr::pull(is_expected) %>% # expect all strata to evaluate to TRUE
    all() %>%
    expect_true()
