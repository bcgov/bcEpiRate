test_that("function works when all arguments are valid", {
  ...
  expect_equal(get_smr(count, popn, std_rt),
               ...
               tolerance = 1e-6)
  expect_equal(get_smr(count, popn, std_rt, output_type = "counts"),
               popn * std_rt)
...
test_that("function throws an error when the input array isn't numeric", {
  ...
  expect_error(get_smr(count, popn, std_rt = char_vec),
...
test_that("function throws an error when `count` isn't a numeric scalar", {
  char <- "apple"
  char_vec <- c("apple", "banana", "carrot")
  counts <- c(15300, 147000)
  popn <- c(1500000, 550000, 120000)
  std_rt <- c(0.0012, 0.0036, 0.048)

  expect_error(get_smr(char, popn, std_rt), "`count` must be numeric")
  expect_error(get_smr(char_vec, popn, std_rt), "`count` must be numeric")
  expect_error(get_smr(counts, popn, std_rt), "`count` must be a scalar")
})
...
test_that("function throws an error when the lengths of input arrays don't match", {
  ...
  expect_error(get_smr(count, popn, std_rt[1:2]),
               "lengths of `popn` and `std_rt` must match")
})

test_that("function throws an error when `output_type` isn't one of 'ratio'
          or 'counts'", {
            count <- 15300
            popn <- c(1500000, 550000, 120000)
            std_rt <- c(0.0012, 0.0036, 0.048)

            expect_error(get_smr(count, popn, std_rt, output_type = "rate"),
...
test_that("function throws an error when `percent` isn't logical", {
  ...
  expect_error(get_smr(count, popn, std_rt, percent = "rate"),
               "`percent` must be one of TRUE or FALSE")
  expect_error(get_smr(count, popn, std_rt, percent = NA),
               "`percent` must be one of TRUE or FALSE")
})

test_that("function throws an error when `output_type` is 'ratio' but input vectors have NA values", {
  count <- 15300
  popn <- c(1500000, 550000, 120000)
  ...
  expect_error(get_smr(count, popn_na, std_rt, output_type = "ratio"),
               "when calculating the SMR, `popn` should not have any NA values")
  expect_error(get_smr(count, popn, std_rt_na, output_type = "ratio"),
...
test_that("function raises a warning when `output_type` is 'counts' and input vectors have NA values", {
  ...
  expect_warning(get_smr(count, popn_na, std_rt, output_type = "counts"),
                 "one or more elements in `popn` are NA")
  expect_equal(suppressWarnings(get_smr(count, popn_na, std_rt, output_type = "counts")),
               popn_na * std_rt,
               tolerance = 1e-6)
  expect_warning(get_smr(count, popn, std_rt_na, output_type = "counts"),
                 "one or more elements in `std_rt` are NA")
  expect_equal(suppressWarnings(get_smr(count, popn, std_rt_na, output_type = "counts")),
               popn * std_rt_na,
               tolerance = 1e-6)
})
